...
onMounted(async () => {
  try {
    const res = await axios.get(`https://pokedexab1-pokedexab.c.aivencloud.com/api/pokemons${route.params.id}`);
    pkm.value = res.data;
  } catch (e) {
    console.error(e);
...
const save = async () => {
  try {
    ...
    await axios.put(`https://pokedexab1-pokedexab.c.aivencloud.com/api/pokemons${pkm.value.id}`, pkm.value);
